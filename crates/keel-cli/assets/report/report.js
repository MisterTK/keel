...
(function () {
  ...
  function boot() {
    ...
    var since = 0, runId = null, events = [], failures = 0;
    function tick() {
      fetch("./api/state?since=" + since, { cache: "no-store" })
        .then(function (r) { if (!r.ok) throw new Error("HTTP " + r.status); return r.json(); })
        .then(function (state) {
          if (state.run && state.run.id !== runId) { runId = state.run.id; events = []; }
          events = events.concat(state.events || []).slice(-EVENT_CAP);
          since = state.events_seq || since;
          failures = 0;
          state.events = events;
          render(state);
          setStalled(false);
  ...
  globalThis.KeelReport = { viewModel: viewModel, render: render, eventLine: eventLine };
