//! A scratch Postgres cluster for `PostgresJournal` integration tests:
//! `initdb` + `pg_ctl` against a throwaway data directory on an unused port,
//! stopped and deleted on [`Drop`].
//!
//! This is a plain module (`tests/support/mod.rs`, not `tests/support.rs`) so
//! Cargo does not treat it as its own integration-test binary — every test
//! file that needs a cluster does `mod support;` and shares this code.
//!
...
//! when none of those resolve, matching the native-front-end-leg convention
//! ("maturin"/napi builds skip cleanly when absent") — so `cargo test` stays
//! green on a machine (or CI image) with no local Postgres.
