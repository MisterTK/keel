...
export async function installKeel({ cwd = process.cwd(), env = process.env } = {}) {
  ...
  const { policy, source } = loadPolicy(cwd); // throws KEEL-E001 on bad syntax
  ...
  banner(env, source, wrappable.length);
  return { enabled: true, backend, discovery, functionTargets, uninstallFetch };
}
...
function banner(env, source, fnCount) {
