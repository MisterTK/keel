...
def _try_load_native(journal_path: str | None) -> Backend | None:
    ...
    if callable(getattr(inst, "configure", None)) and callable(getattr(inst, "execute", None)):
        ...
        # persistent/layer/resolve_target directly (layer + resolve_target
        # bound since Task 9/Task 10 of SP-1) — no wrapper needed; return it
        # as-is, so the swap between native and stub is transparent.
