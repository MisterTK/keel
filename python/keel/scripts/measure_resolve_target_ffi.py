...
def measure() -> dict[str, Any]:
    """`{native_loaded, resolve_target_ns, layer_ns}` — the latter two `None`
    when the native wheel isn't built (nothing to measure)."""
    try:
        import keel_core
    except ImportError:
        return {"native_loaded": False, "resolve_target_ns": None, "layer_ns": None}

    core = keel_core.KeelCore()
    resolve_target_ns = median_ns(lambda: core.resolve_target("GET", "api.openai.com"))
    layer_ns = median_ns(lambda: core.layer("llm:openai", "retry"))
    return {"native_loaded": True, "resolve_target_ns": resolve_target_ns, "layer_ns": layer_ns}
...
def format_summary(result: dict[str, Any]) -> str:
    ...
    return (
        f"[resolve_target ffi] python resolve_target {result['resolve_target_ns']} ns "
        f"| layer {result['layer_ns']} ns"
    )
